...
var accionUbicacion = function(){
  ...
  var obtener_tiposubi = function(data){
    var combo = $('[name=tipo_ubicacion]');
    $.ajax({
        url: "Instructores_controller/obtener_tiposubi",
 ...
 obtener_ubicacion = function(){
      ...
      var modelFila = '<tr>'+
          ...
          '    </tr>';
      $.ajax({
          url: 'Instructores_controller/obtener_ubicacion',
          type: 'POST',
          data: {
              codigo: codigo
          },
          success: function(response) {
              ...
              if (respuesta.success === true) {
                ...
                $.notify({
                    message: "Este Instructor no posee ninguna ubicación, por favor ingrese una."
                }, {
                    type: 'danger',
                    delay: 1000,
                    placement: {
              ...
              waitingDialog.hide();
              $('#nombre_clienteU').html('<p> <strong> Nombres : ' + nombre + ' </strong> </p>')
          }
      });
  };
    ...
    if (invalidDelete === true) {
      ...
      bootbox.confirm({
        ...
        callback: function (result) {
          if(result === true){
            $.ajax({
              url: "Instructores_controller/inactivar_ubicacion",
              type:"POST",
              data: {
...
var guardar_barrio = function(){
  ...
  if (error === true) {
    ...
    var fd = new FormData(document.getElementById("formBarrio"));
        ...
        $.ajax({
          url: 'Instructores_controller/guardar_barrio',
          type: 'POST',
          data: fd,
...
function obtener_barrios(data){
  ...
  $.ajax({
    url: "Instructores_controller/obtener_barrios",
    type: 'POST',
    data: {
...
function validar_ubipri() {
  ...
  $.ajax({
    asyc: false,
    url: 'Instructores_controller/validar_ubipri',
    type: 'POST',
    data: {
      tipo: tipo,
      tercero: tercero
...
var guardarUbicacion = function(){
  ...
  if (error === true) {
    ...
    btnGuardar.attr("disabled", "disabled");
    $.ajax({
      url: 'Instructores_controller/guardar_ubicacion',
      type: 'POST',
      data: fd,
      processData: false, // tell jQuery not to process the data
      contentType: false   // tell jQuery not to set contentType
