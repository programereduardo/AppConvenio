<?php
  ...
  class usuarios_model extends CI_model
  {
    ...
    public function obtener_usuarios($data)
    {
      ...
      $this->db->select('seg_usuarios.usu_usuario');
      $this->db->select('seg_usuarios.usu_estado');
      $this->db->select('seg_usuarios.usu_pin');
      ...
      $this->db->join('gloterceros', 'seg_usuarios.usu_tercero = gloterceros.tercodigo', 'left');
      ...
      if ($data !== "Administrador") {
        $this->db->where('seg_roles.rol_nombre', 'Instructor');
      }
      $result = $this->db->get();
